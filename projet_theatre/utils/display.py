...
def refreshGenericineEdit(lineEdit, result):
    lineEdit.clear()
    for row_num, row_data in enumerate(result):
        lineEdit.setText(str(row_data[0]))

def refreshGenericComboAndLineEdit(combo, lineEdit,result):
    combo.clear()
    for row_num, row_data in enumerate(result):
        combo.addItem(str(row_data[0]))
        lineEdit.setText(str(row_data[1]))
